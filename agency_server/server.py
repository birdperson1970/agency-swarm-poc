...
from agency_swarm import demo_gradio

from langchain.agents import load_tools


logger.info(
    f"GOOGLE_CSE_ID={os.getenv('GOOGLE_CSE_ID')}  GOOGLE_API_KEY={os.getenv('GOOGLE_API_KEY')}"
)
        ...
        self._agency = Agency(
            [
                ...
                [
                    agent_po,
                    agent_va,
                ],  # CEO can initiate communication with Virtual Assistant
                [agent_po, agent_arch],
                [agent_arch, agent_dev],
                [agent_dev, agent_arch],
            ],
            shared_instructions="agents/manifesto.md",
        )  # shared instructions for all agents

    async def run(self):
        demo_gradio(self._agency, height=900)
...
def fetch_threads():
    ...
    return [
        {"thread_id": 1, "thread_title": "Thread A", "thread_content": "Content A"},
...
def display_threads(thread_data):
    ...
    return "".join(
        [
            f"ID: {thread['thread_id']} Title: {thread['thread_title']} Content: {thread['thread_content']}"
...
threads_interface = gr.Interface(
    fn=display_threads, inputs="textbox", outputs="text", title="Thread Display"
)
