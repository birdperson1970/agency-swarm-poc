...
class Thread:
    ...
    def get_completion(self, message: str, yield_messages=True):
        if not self.thread:
            ...
            print(
                f"THREAD:[ {sender_name} -> {self.recipient_agent.name} ]: URL {playground_url}"
            )
        ...
        if yield_messages:
            yield MessageOutput(
                "text", self.agent.name, self.recipient_agent.name, message
            )
        ...
        while True:
            ...
            if self.run.status == "requires_action":
                ...
                for tool_call in tool_calls:
                    tool_str = f"\n-------------- REQUEST {tool_call.function.name} [{tool_call.id}] ------------\n"
                    arguments = json.loads(tool_call.function.arguments)
                    for arg in arguments:
                        tool_str += f"{arg}: {arguments[arg]}\n"
                    tool_str += separator
                    ...
                    tool_str = f"\n-------------- RESPONSE {tool_call.function.name} [{tool_call.id}] ------------\n"
                    tool_str += output + "\n"
                    tool_str += separator
                    logger.info(tool_str)
            ...
            # return assistant message
                ...
                if yield_messages:
                    yield MessageOutput(
                        "text", self.recipient_agent.name, self.agent.name, message
                    )
    ...
    def _execute_tool(self, tool_call):
        ...
        func = next(
            (func for func in funcs if func.__name__ == tool_call.function.name), None
        )
